...
id_face_buffer = defaultdict(list)       # 每个id的缓存
final_age_gender_race = dict()           # 最终结果
buffer_max_len = 5                       # 缓冲区长度(帧数)，可以自己调
window_size = 5
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        current_time = dataset.frame / fps
            ...
            for t in online_targets:
                ...
                if tid not in final_age_gender_race:
                    ...
                    if len(face_results) > 0:
                        ...
                        try:
                            ...
                            face_crop = torch.from_numpy(face_crop).permute(2, 0, 1).unsqueeze(0).float().to(device)

                            with torch.no_grad():
                                output = fairface_model(face_crop)
                            ...
                            id_face_buffer[tid].append((age, gender, race))
                            ...
                            if len(id_face_buffer[tid]) >= buffer_max_len:
                                ...
                                final_race = Counter(races).most_common(1)[0][0]
                                final_age_gender_race[tid] = (final_age, final_gender, final_race)

                        except Exception as e:
                            print(f'Face crop error for tid {tid}: {e}')
                    else:
                ...
                if tlwh[2] * tlwh[3] > opt.min_box_area:
                    ...
                    if save_img or view_img:  # Add bbox to image
                        age, gender, race = final_age_gender_race.get(tid, (-1, -1, -1))
                        ...
                        label = f'ID:{tid} {gender_str} {age_str} {race_str}'
    ...
    flow_res_file = 'runs/track/flow_count.txt'
    os.makedirs('runs/track/', exist_ok=True)
    res_file = 'runs/track/stay_times.txt'
    long_stay_res_file = 'runs/track/stay_long_analysis.txt'
    
    
    with open(flow_res_file, 'w') as f:
        f.write("Time Range, Number of People\n")
        f.writelines(flow_records)
    print(f"Flow count records saved to {flow_res_file}")

    with open(res_file, 'w') as f:
        f.write("ID, Entry Time (s), Exit Time (s), Stay Duration (s), Age, Gender, Race\n")
        stay_long_ids = []
        for tid, (enter, leave) in id_records.items():
            ...
            f.write(f"{tid},{enter:.2f},{leave:.2f},{stay_duration:.2f},{age_str},{gender_str},{race_str}\n")

            if stay_duration >= 2:
                stay_long_ids.append(tid)
    print(f"Stay duration records saved to {res_file}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='Pretrained/yolov7.pt', help='model.pt path(s)')
