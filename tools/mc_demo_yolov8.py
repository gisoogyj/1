                ...
                if tid not in final_age_gender_race:
                    ...
                    if face_results:
                        ...
                        try:
                            ...
                            gender_conf = float(output[7:9][gender_idx])
                            age_conf = float(output[9:18][age_idx])
                            race_conf = float(output[:7][race_idx])
                            overall_conf = min(gender_conf, age_conf, race_conf)                           
                            ...
                            id_face_buffer[tid].append((age_idx, gender_idx, race_idx, overall_conf))

                            # if len(id_face_buffer[tid]) >= buffer_max_len:
                            #     best_record = max(id_face_buffer[tid], key=lambda x: x[3])
                            #     best_age_idx, best_gender_idx, best_race_idx, _ = best_record

                            #     final_age = age_map[best_age_idx] if 0 <= best_age_idx < len(age_map) else 'U'
                            #     final_gender = gender_map[best_gender_idx] if best_gender_idx in [0, 1] else 'U'
                            #     final_race = race_map[best_race_idx] if 0 <= best_race_idx < len(race_map) else 'U'

                            #     final_age_gender_race[tid] = (final_age, final_gender, final_race)
                            if len(id_face_buffer[tid]) >= buffer_max_len:
                                # 先过滤掉 (U, U, U) 的帧
                                valid_records = [record for record in id_face_buffer[tid] if not (record[0] == -1 and record[1] == -1 and record[2] == -1)]

                                if valid_records:
                                    # 在正常记录中，取置信度最大的帧
                                    best_record = max(valid_records, key=lambda x: x[3])
                                else:
                                    # 如果没有正常预测，还是从所有记录里选一条
                                    best_record = max(id_face_buffer[tid], key=lambda x: x[3])
