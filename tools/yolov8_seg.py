...
def detect(opt):
    ...
    for img_path in img_files:
        ...
        person_boxes = []
        person_masks = []
        for box, mask, cls_id in zip(boxes, masks, cls_ids):
            if int(cls_id) == 0:
                person_boxes.append(box)
                person_masks.append(mask)

        # Remove duplicate boxes
        filtered_boxes, filtered_masks = [], []
        contain_thresh = 0.85
        for i in range(len(person_boxes)):
            ...
            for j in range(len(filtered_boxes)):
                ...
                if max(contain1, contain2) > contain_thresh:
                    print(f"Removing box {i}, overlap with box {j} at max contain {max(contain1, contain2):.2f}")
                    keep = False
                    break
            if keep:
        ...
        for box, mask in zip(filtered_boxes, filtered_masks):
            ...
            if face_results:
                ...
                try:
                    ...
                    label = f"{person_id}:{gender_str}"
                    plot_one_box([x1, y1, x2, y2], im0, label=label, color=colors[person_id % len(colors)], line_thickness=2)
                    colored_mask = np.zeros_like(im0)
                    color = (0, 255, 0)
                    for c in range(3):
                        colored_mask[:, :, c] = mask_bin * color[c] // 255
                    im0 = cv2.addWeighted(im0, 1.0, colored_mask, 0.5, 0)
                    results.append([img_path.name, person_id, gender_str, age_str, race_str])
                    person_id += 1
